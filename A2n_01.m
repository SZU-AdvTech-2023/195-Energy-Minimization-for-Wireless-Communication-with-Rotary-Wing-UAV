...
% T_last((M+2)/4)=Qk/(log2(1+gama_k/((H^2+norm(q_last((M+2)/4,:)-wk_t(1,:))^2)^(a/2))));
% T_last((M+2)*2/4)=Qk/(log2(1+gama_k/((H^2+norm(q_last((M+2)*2/4,:)-wk_t(2,:))^2)^(a/2))));
% T_last((M+2)*3/4)=Qk/(log2(1+gama_k/((H^2+norm(q_last((M+2)*3/4,:)-wk_t(3,:))^2)^(a/2))));
% T_last((M+2)/4)=30;
% T_last((M+2)*2/4)=30;
% T_last((M+2)*3/4)=30;%Too large to cal
...
for i=1:K_GN
    for m = 1:M+1
        A_last(m,i) = sqrt(tao_last(m,i)*...
            log2(1+gama_k/(H^2 + norm(q_last(m,:) - wk_t(i,:))^2)^(a/2)));
    end
end
    ...
    for i=1:K_GN
        for m=1:(M+1)
            beta_mk(m,i)=(log2(exp(1))*gama_k*(a/2))/...
                ...
                ((H^2+norm(q_last(m,:)-wk_t(i,:))^(a/2)+gama_k)));
            current_rate(m,i)=log2(1+gama_k/(H^2+norm(q_last(m,:)-wk_t(i,:))^(a/2)));
        end
    end
...
hold on
plot(q(:,1),q(:,2),...
        'Color','r','LineWidth',1),title('Q = 0.1Mbits');
hold off
plot_v=zeros(M+1,1);
for m=1:M+1
    plot_v(m)=norm(q(m+1,:)-q(m,:))/T(m);
end
save("0.1v.mat","plot_v","T");