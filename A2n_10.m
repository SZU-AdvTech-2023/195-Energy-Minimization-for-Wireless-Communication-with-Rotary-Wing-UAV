    ...
    for i=1:K_GN
        for m=1:(M+1)
            beta_mk(m,i)=(log2(exp(1))*gama_k*(a/2))/...
                ...
                ((H^2+norm(q_last(m,:)-wk_t(i,:))^2)+gama_k));
            current_rate(m,i)=log2(1+gama_k/(H^2+norm(q_last(m,:)-wk_t(i,:))^2));
        end
    end
...
hold on
plot(q(:,1),q(:,2),...
        'Color','r','LineWidth',1),title('Q = 10Mbits');
hold off
plot_v=zeros(M+1,1);
for m=1:M+1
    plot_v(m)=norm(q(m+1,:)-q(m,:))/T(m);
end
save("10v.mat","plot_v","T");